...
function isTriangle(a, b, c) {
  const array = [a, b, c].sort((x, y) => y - x);
...
function isCreditCardNumber(ccn) {
  const arr = Array.from(ccn.toString(), Number);
  if (arr.length % 2 === 0) {
  ...
  const res = arr.reduce((acc, cv) => acc + cv, 0);
  return res % 10 === 0;
}
...
function getDigitalRoot(num) {
  const str = String(num).split('').reduce((a, b) => a + Number(b), 0);
  return str > 9 ? getDigitalRoot(str) : str;
}
...
function getCommonDirectoryPath(pathes) {
  const arr = pathes;
  const newArr = [];
  let minLength = pathes[0].split('/').length;
